...
t = par.AnatGrps;
ngroups = length(t);
tgroups = cell(ngroups,1);
for g = 1:ngroups
    tgroups{g} = t(g).Channels;
end
...
switch(electrode_type)
    ...
    case 'A16'
        disp('A16 probe layout')
            x = zeros(1,length(tgroups{1}));
            y = zeros(1,length(tgroups{1}));
            for i =1:length(tgroups{1})
                y(i) = -i*50;
            end
            xcoords = x;
            ycoords = y;
...
switch(electrode_type)
    case {'staggered','poly3','poly5','A16'}
        for a= 1:ngroups
